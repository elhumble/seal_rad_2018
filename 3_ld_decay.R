...
system("plink --bfile data/ld/GQ_DP --extract data/ld/snps_top100.txt --nonfounders --recode --make-bed --out data/ld/top40snps --allow-extra-chr")

# Remove SNPs with Mendel Errors
system("plink --bfile data/ld/top100snps --exclude data/mendel/MendelErrorStrict_SNPs.txt --nonfounders --recode --make-bed --out data/ld/top40snpsME --allow-extra-chr")
...
# Filter for 50% genotyping rate and 1% MAF
# geno 0.01 = 99%
# geno 0.05 = 95%
# geno 0.1 = 90%
...
for (i in 1:length(pop_files)){
  system(paste0("plink --bfile data/ld/top40snpsME --keep ", pop_files[i], " --nonfounders --geno 0.5 --maf 0.1 --hwe 0.001 --make-bed --out data/ld/", out_names[i], " --allow-extra-chr"))
}
...
nonlinear_ld_curve <- function(r2){
  ...
  LD.data <- r2$R2
  n <- length(unique(c(r2$CHR_A, r2$CHR_B)))
  #ggplotdf <- as.data.frame(cbind(distance, LD.data))
...
jpeg(file="figs/LD_decay_SSB.jpg", units = "in", res = 300, height=6, width=7)
...
ssb_ld <- filter(ld_out_df, .id == "SSB")
mean(ssb_ld$R2)

ssb_curves <- filter(ld_curves, .id == "SSB")
